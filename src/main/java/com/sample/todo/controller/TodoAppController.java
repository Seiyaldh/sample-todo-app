...
public class TodoAppController {
    ...
    // 検索時の空検証と結果に応じたリストとエラーメッセージの取得
    public static String valEmptyTodoList(List<TodoApp> todoList, MessageSource msg, TodoAppService service,
            Model model) {
    ...
    String register(@RequestParam String title, String detail, String dueDate,
            @Validated @ModelAttribute TodoApp todoApp, BindingResult result, Model model) {
        if (result.hasErrors()) {
    ...
    String updater(@RequestParam int todoId, String title, String detail, String dueDate,
            @Validated @ModelAttribute TodoApp todoApp, BindingResult result, Model model) {
        if (result.hasErrors()) {
            List<String> errorList = getErrorList(result, msg);
            model.addAttribute("validationError", errorList);
            addParmUpdate(todoId, title, detail, dueDate, model);
            return "update";
        } // もしエラーが吐かれたらエラーメッセージを表示しupdateに戻る
    ...
    String searchDueDate(@Validated @ModelAttribute TodoApp todoApp, BindingResult result, Model model) {
        ...
        if (result.hasErrors()) {
            for (ObjectError error : result.getAllErrors()) {
                if (error.getDefaultMessage().matches(".*(dueDate).*")
                        || error.getDefaultMessage().matches(".*(startDueDate).*")) {
                    todoList = service.getTodoAppList();
                    model.addAttribute("todoList", todoList);
                    String noHitMsg = msg.getMessage("date_error_key", null, Locale.JAPAN);// dueDate関連でエラーが起きたら自作のエラーメッセージを取得
                    model.addAttribute("noHitMsg", noHitMsg);
                    List<TodoApp> mostRecentTodoList = service.getMostRecentTodoAppList();
                    model.addAttribute("mostRecentTodoList", mostRecentTodoList);
                    return "index";
        ...
        } // もしエラーが吐かれたらエラーメッセージを表示しindexに戻る
        return valEmptyTodoList(todoList, msg, service, model);
    }
}
